...
import { GripVertical, Eye, EyeOff, Save, Monitor } from 'lucide-react';
...
const AdminNavigation = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data) {
        ...
        setTopbarSettings(data.topbar_settings || { height: 'standard' });
      } else {
          ...
          .insert([
            {
              navigation_items: [],
              footer_settings: {},
              topbar_settings: { height: 'standard' }
            }
          ])
          .select('id')
          .single();
        ...
        setTopbarSettings({ height: 'standard' });
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      // Обновляем страницу для применения изменений топбара
      setTimeout(() => {
  ...
  const heightOptions = [
    {
      value: 'compact' as TopbarHeight,
      label: 'Компактная',
      description: 'Минимальная высота топбара (py-1)',
      preview: '~32px'
    },
    {
      value: 'standard' as TopbarHeight,
      label: 'Стандартная',
      description: 'Обычная высота топбара (py-2)',
      preview: '~56px'
    },
    {
      value: 'large' as TopbarHeight,
      label: 'Большая',
      description: 'Увеличенная высота топбара (py-4)',
      preview: '~80px'
    }
  ];
        ...
        <div className="p-6">
            ...
            {heightOptions.map((option) => (
              <label
                ...
                className={`flex flex-col p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  topbarSettings.height === option.value
              ...
              >
                <input
                  ...
                  name="height"
                  value={option.value}
                  checked={topbarSettings.height === option.value}
                  onChange={(e) => setTopbarSettings({ 
                    height: e.target.value as TopbarHeight 
                  })}
                  className="sr-only"
                />
          ...
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <p className="text-sm text-blue-700 dark:text-blue-300">
              <strong>Внимание:</strong> После сохранения настроек страница будет перезагружена для применения изменений высоты топбара.
            </p>
          </div>
        </div>
      </div>
