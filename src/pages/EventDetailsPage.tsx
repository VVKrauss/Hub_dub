                ...
                {!isEventPast && (
                  <EventRegistrationBlock
                    event={{
                      ...
                      price: event.price || 0,
                      currency: event.currency || 'RSD',
                      simple_payment_type: event.payment_type === 'free' ? 'free' : 
                                         event.payment_type === 'donation' ? 'donation' : 'paid',
                      online_payment_url: event.oblakkarte_data_event_id || event.payment_link,
                      online_payment_type: event.oblakkarte_data_event_id ? 'oblakkarte' : 
                                          event.payment_link ? 'link' : undefined,
                      max_registrations: event.registrations?.max_regs || 50,
                      current_registrations: event.registrations?.current || 0,
                      registration_enabled: true, // По умолчанию включено
                      registration_deadline: undefined // Пока нет в старой структуре
                    }}
                    onRegistrationSuccess={handleRegistrationSuccess}
                  />
                )}
