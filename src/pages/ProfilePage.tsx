...
import { User, Mail, Calendar, Edit, Save, X, Shield, Heart, Users, ExternalLink, MapPin, Clock } from 'lucide-react';
              ...
              {loadingFavorites ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {favoriteSpeakersData.map((speaker) => (
                    <div key={speaker.id} className="border border-gray-200 dark:border-dark-600 rounded-lg hover:shadow-md transition-shadow relative group">
                      {/* Кнопка удаления */}
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          removeFavoriteSpeaker(speaker.id);
                        }}
                        className="absolute top-3 right-3 z-10 text-red-500 hover:text-red-700 bg-white dark:bg-dark-800 rounded-full p-1 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity"
                      ...
                      <Link to={`/speakers/${speaker.id}`} className="block p-4">
                        <div className="flex items-start justify-between mb-3 pr-8">
                          <div className="flex-1">
                            <h4 className="font-semibold text-lg hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                              {speaker.name}
                            </h4>
              ...
              {loadingFavorites ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {favoriteEventsData.map((event) => (
                    <div key={event.id} className="border border-gray-200 dark:border-dark-600 rounded-lg overflow-hidden hover:shadow-md transition-shadow relative group">
                      {/* Кнопка удаления */}
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                      ...
                      {/* Изображение мероприятия или заглушка */}
                      ...
                      <Link to={`/events/${event.id}`} className="block p-4">
                        <h4 className="font-semibold text-lg line-clamp-2 mb-2 hover:text-primary-600 dark:hover:text-primary-400 transition-colors pr-8">
                          {event.title}
                        </h4>
                        ...
                        <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                          ...
                          {event.event_type && (
                            <div className="inline-block px-2 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-200 rounded text-xs">
                              {event.event_type}
                            </div>
                          )}
                        </div>
            ...
            <div className="p-6">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                <p className="text-sm text-blue-800 dark:text-blue-200">
                  🚧 <strong>В разработке:</strong> Избранные типы мероприятий и история регистраций будут добавлены в следующих обновлениях.
                </p>
              </div>
            </div>
          </div>
