  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-dark-900">
      ...
      <div className="hidden md:block fixed top-0 left-0 right-0 h-12 bg-white dark:bg-dark-800 border-b border-gray-200 dark:border-dark-700 z-40 shadow-sm">
        <div className="flex items-center justify-between h-full px-4">
          <div className={`transition-all duration-300 ${isSidebarCollapsed ? 'ml-16' : 'ml-64'} flex items-center gap-6`}>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-black bg-opacity-50" onClick={closeMobileMenu}>
          <div className="fixed inset-y-0 left-0 w-64 bg-white dark:bg-dark-800 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-dark-700">
              ...
              <button
                ...
                className="p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-dark-700 rounded"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
      ...
      {/* Sidebar - Desktop */}
      <aside className={`
        hidden md:block fixed left-0 top-12 bottom-0 z-30 transition-all duration-300 ease-in-out
        ${isSidebarCollapsed ? 'w-16' : 'w-64'}
        bg-white dark:bg-dark-800 border-r border-gray-200 dark:border-dark-700 shadow-sm
      `}>
        <div className="h-full flex flex-col">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {showLabel && <span className="text-sm truncate">{item.shortLabel}</span>}
          ...
          <div className="p-3 border-t border-gray-200 dark:border-dark-700">
            <button
              onClick={handleLogout}
              className={`
                w-full flex items-center gap-2 px-2 py-2.5 rounded-md text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors group relative
                ${isSidebarCollapsed ? 'justify-center' : ''}
              `}
              title={isSidebarCollapsed ? 'Выйти' : ''}
            >
              <LogOut className="h-4 w-4 flex-shrink-0" />
              {!isSidebarCollapsed && <span className="text-sm">Выйти</span>}
