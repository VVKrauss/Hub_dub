-- Migration: Simplified Registration System
-- Description: Creates a simplified registration system with better performance and easier management

-- 1. Создаем новую упрощенную таблицу регистраций
CREATE TABLE IF NOT EXISTS simple_registrations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id TEXT NOT NULL, -- Changed from UUID to TEXT to match events.id
  ...
  -- Индексы
  CONSTRAINT simple_registrations_qr_code_key UNIQUE(qr_code),
  CONSTRAINT fk_simple_registrations_event FOREIGN KEY (event_id) REFERENCES events(id) ON DELETE CASCADE
);

-- Создаем индексы для оптимизации запросов
CREATE INDEX IF NOT EXISTS idx_simple_registrations_event_id ON simple_registrations(event_id);
CREATE INDEX IF NOT EXISTS idx_simple_registrations_email ON simple_registrations(email);
CREATE INDEX IF NOT EXISTS idx_simple_registrations_qr_code ON simple_registrations(qr_code);
CREATE INDEX IF NOT EXISTS idx_simple_registrations_status ON simple_registrations(registration_status, payment_status);
...
CREATE OR REPLACE FUNCTION get_event_registrations(p_event_id TEXT)
RETURNS TABLE (
  id UUID,
  full_name TEXT,
  email TEXT,
